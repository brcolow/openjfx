...
public abstract class GlassRobot {
    ...
    public final WritableImage getScreenCapture(double x, double y, double width, double height, boolean scaleToFit) {
        javafx.stage.Screen primaryScreen = Screen.getPrimary();
        ...
        if (outputScaleX == 1.0f && outputScaleY == 1.0f) {
            ...
            getScreenCapture((int) x, (int) y, (int) width, (int) height, data);
            dw = (int) width;
            dh = (int) height;
        } else {
        ...
        return convertFromPixels(Application.GetApplication().createPixels(dw, dh, IntBuffer.wrap(data)));
    }
